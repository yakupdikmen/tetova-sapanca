    ...
    <section className="relative z-20 min-h-[92vh] flex items-center justify-center py-24 px-4 sm:px-6 lg:px-8">
      ...
      <div className="relative z-10 w-full max-w-6xl mx-auto text-center flex flex-col items-center">
        ...
        >
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full backdrop-blur-md bg-amber-500/20 border border-amber-500/30 text-amber-300 text-xs sm:text-sm font-semibold tracking-wide uppercase shadow-lg">
          ...
          <motion.p
            variants={itemVariants}
            className="text-stone-200 font-normal text-base sm:text-lg md:text-xl max-w-2xl mx-auto drop-shadow-sm mt-2 mb-8 leading-relaxed"
          >
            {t("hero.subtitle")}
          </motion.p>
          ...
          <motion.div
            variants={itemVariants}
            className="w-full max-w-5xl bg-stone-900/60 dark:bg-stone-900/80 backdrop-blur-xl border border-white/15 rounded-3xl p-4 md:p-6 shadow-2xl ltr:text-left rtl:text-right"
          >
            <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-12 gap-4 items-end">
              ...
              <div className="lg:col-span-4 flex flex-col gap-2">
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full bg-stone-800/80 border border-white/10 rounded-xl px-4 py-3 text-sm font-semibold text-white focus:outline-none focus:ring-2 focus:ring-amber-500/50 appearance-none cursor-pointer"
                  >
                    {BUNGALOW_CATEGORIES.map((cat) => (
                      <option key={cat.id} value={cat.id} className="bg-stone-900 text-white font-semibold">
                        {getCategoryName(cat.id)}
                      </option>
                    ))}
                  </select>
                  <div className="pointer-events-none absolute inset-y-0 ltr:right-0 rtl:left-0 flex items-center px-3 text-stone-400">
              ...
              <div className="lg:col-span-12 mt-2 pt-2 border-t border-white/10 flex ltr:justify-end rtl:justify-start">
                <button
                  type="submit"
                  className="w-full sm:w-auto px-8 py-3.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-stone-950 font-extrabold text-sm tracking-wide shadow-lg shadow-amber-500/25 transition-all duration-300 flex items-center justify-center gap-2 transform hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.98] cursor-pointer"
                >
