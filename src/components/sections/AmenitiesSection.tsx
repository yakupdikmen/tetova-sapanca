...
export const AmenityCard: React.FC<{ title: string; desc: string; iconName: string; index: number }> = ({
  ...
  return (
    <motion.div
      ...
      transition={SPRING_TRANSITION}
      className="group relative flex flex-col justify-between p-6 sm:p-8 rounded-3xl bg-white/80 dark:bg-stone-900/40 backdrop-blur-md border border-amber-900/10 dark:border-white/5 hover:border-amber-500/30 transition-all duration-300 shadow-lg hover:shadow-[0_20px_40px_-15px_rgba(245,158,11,0.15)] ltr:text-left rtl:text-right"
    >
      <div>
        {/* Top Icon & Tag Bar */}
        <div className="flex items-center justify-between mb-6">
          ...
          <motion.div
            ...
            transition={SPRING_TRANSITION}
            className="w-14 h-14 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-600 dark:text-amber-400 group-hover:bg-amber-500 group-hover:text-stone-950 transition-all duration-300 shadow-md"
          >
        ...
        {/* Title & Description */}
        <h3 className="text-xl font-bold text-stone-900 dark:text-white tracking-tight group-hover:text-amber-600 dark:group-hover:text-amber-300 transition-colors">
          {title}
        </h3>
        <p className="text-stone-600 dark:text-stone-300 text-sm mt-2 leading-relaxed font-normal">
          {desc}
        </p>
      </div>

      {/* Decorative Bottom Bar */}
      <div className="w-12 h-1 rounded-full bg-amber-500/20 group-hover:w-full group-hover:bg-amber-500 transition-all duration-500 mt-6" />
    </motion.div>
  );
};
...
export const AmenitiesSection: React.FC<AmenitiesSectionProps> = () => {
  ...
  return (
    <section className="relative bg-[#FDFBF7] dark:bg-stone-950 py-24 px-4 sm:px-6 lg:px-8 overflow-hidden text-stone-900 dark:text-white border-t border-amber-900/5 dark:border-white/5 transition-colors duration-300">
      ...
      <div className="absolute top-1/3 right-10 w-96 h-96 bg-amber-500/10 dark:bg-amber-950/20 rounded-full filter blur-[140px] pointer-events-none" />
      <div className="absolute bottom-10 left-10 w-96 h-96 bg-orange-500/10 dark:bg-orange-950/20 rounded-full filter blur-[140px] pointer-events-none" />
