...
export const AmenityCard: React.FC<AmenityCardProps> = ({ amenity }) => {
  ...
  return (
    <motion.div
      ...
      transition={SPRING_TRANSITION}
      className="group relative flex flex-col justify-between p-6 sm:p-8 rounded-3xl bg-white/80 dark:bg-slate-900/40 backdrop-blur-md border border-amber-900/10 dark:border-white/5 hover:border-emerald-500/30 transition-all duration-300 shadow-lg hover:shadow-[0_20px_40px_-15px_rgba(16,185,129,0.15)]"
    >
      <div>
        {/* Top Icon & Tag Bar */}
        <div className="flex items-center justify-between mb-6">
          ...
          {amenity.highlightTag && (
            <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-xs font-semibold uppercase tracking-wider">
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full backdrop-blur-md bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-xs font-semibold uppercase tracking-widest mb-4">
            ...
            <span>KUSURSUZ KONFOR & LÜKS AYRICALIKLAR</span>
          </div>
          <h2 className="text-3xl sm:text-5xl font-extrabold text-slate-900 dark:text-white tracking-tight leading-tight">
            Sapanca Vista ile <br className="hidden sm:inline" />
          ...
          <p className="text-slate-600 dark:text-slate-300 text-base sm:text-lg mt-4 font-normal leading-relaxed">
            Her mevsim sıcak kalan havuzlardan yüksek mahremiyetli bahçelere kadar tatilinizi ayrıcalıklı kılacak tüm detaylar düşünüldü.
          </p>
        </div>
