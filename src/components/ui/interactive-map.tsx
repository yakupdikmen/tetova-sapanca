...
const createPoiMarkerIcon = (spotName: string, isSelected: boolean) => {
  const bgClass = isSelected
    ? "bg-emerald-600 border-white text-white scale-125 shadow-[0_0_20px_rgba(16,185,129,0.9)]"
  ...
  return L.divIcon({
    ...
    html: `
      <div class="relative flex flex-col items-center group cursor-pointer transition-all duration-300">
        ${isSelected ? '<span class="absolute w-9 h-9 bg-emerald-500/50 rounded-full animate-ping"></span>' : ""}
        <div class="w-8 h-8 rounded-full flex items-center justify-center border-2 shadow-xl ${bgClass}">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" stroke="none"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3" fill="#0f172a"/></svg>
        </div>
        <span class="mt-1 px-2 py-0.5 rounded-md text-[10px] font-bold whitespace-nowrap shadow-md ${
          isSelected
            ...
            : "bg-slate-950/90 text-slate-100 border border-white/10"
        }">
          ${spotName}
        </span>
      </div>
...
export const AdvancedMap: React.FC<AdvancedMapProps> = ({
  ...
  return (
    <div className={`relative w-full h-[600px] rounded-3xl overflow-hidden shadow-2xl border border-slate-200/80 dark:border-white/10 ${className}`}>
      <MapContainer
      ...
      >
          ...
          <Popup className="custom-leaflet-popup">
            <div className="p-3 max-w-xs text-slate-900">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-600 text-white font-bold text-xs hover:bg-emerald-500 transition-colors shadow-md"
              >
        ...
        {destinations.map((spot) => {
          ...
          return (
            ...
            >
              <Popup className="custom-leaflet-popup">
                <div className="p-2.5 max-w-xs flex gap-3 items-center text-slate-900">
                  <img
                    src={spot.image}
                    alt={spot.name}
