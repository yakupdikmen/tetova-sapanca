    ...
    <AnimatePresence>
      {selectedBungalow && (
        <motion.div
          ...
          className="fixed inset-0 z-50 bg-stone-950/90 backdrop-blur-xl flex items-center justify-center p-3 sm:p-6 md:p-10 overflow-y-auto"
          onClick={onClose}
        >
          ...
          <motion.div
            ...
            className="relative bg-stone-900/90 border border-white/10 rounded-3xl max-w-5xl w-full max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col lg:flex-row my-auto"
          >
            ...
            <motion.button
              ...
              transition={SPRING_TRANSITION}
              onClick={onClose}
              className="absolute top-4 right-4 z-20 w-10 h-10 rounded-full bg-stone-950/80 border border-white/15 text-stone-300 hover:text-white flex items-center justify-center backdrop-blur-md shadow-lg cursor-pointer"
            >
              <X className="w-5 h-5" />
            </motion.button>

            {/* Left/Top: Image & Gallery Section (60% width) */}
            <div className="lg:w-[60%] flex flex-col p-4 sm:p-6 bg-stone-950/50 border-b lg:border-b-0 lg:border-r border-white/10">
              {/* Main Image with Shared layoutId */}
              <div className="relative rounded-2xl overflow-hidden aspect-[4/3] bg-stone-950 flex items-center justify-center">
                <motion.img
                ...
                {/* Rating Badge */}
                <div className="absolute top-4 left-4 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full backdrop-blur-md bg-stone-950/70 border border-white/15 text-amber-400 text-xs font-semibold shadow-lg">
            ...
            <div className="lg:w-[40%] p-6 sm:p-8 flex flex-col justify-between text-white">
              <div className="flex flex-col gap-5">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    {selectedBungalow.featured && (
                      <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-amber-500/20 border border-amber-400/30 text-amber-300 text-[10px] font-bold uppercase tracking-wider">
                        <Sparkles className="w-3 h-3 text-amber-300" />
                        VIP SEÇENEK
                      </span>
                    )}
                    <span className="inline-flex items-center gap-1 text-stone-300 text-xs">
                ...
                {/* Specs Grid */}
                <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-2 gap-2.5 p-3 rounded-2xl bg-stone-950/60 border border-white/5 text-xs text-stone-200">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedBungalow.amenities.map((item, idx) => (
                      <span
                        key={idx}
                        className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-stone-950/70 border border-white/10 text-xs text-stone-200"
                      >
